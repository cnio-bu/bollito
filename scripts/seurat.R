...
try(seurat <- subset(seurat, subset = Epcam > 0 | Pecam1 > 0 | Krt19 > 0 | Ptprc > 0 , invert = TRUE), silent=FALSE)
...
top.markers <- as.matrix(top.markers)
...
saveRDS(seurat, file = paste0(snakemake@output[['data']]))
